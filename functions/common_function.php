...
function get_related()
{
    ...
    if (isset($_GET['product_id'])) {
        if (!isset($_GET['category'])) {
            ...
            $select_query = "Select * from `products` where product_id=$product_id";
            $result_query = mysqli_query($con, $select_query);
            $products_data = mysqli_fetch_assoc($result_query);
            $category_id = $products_data['category_id'];
            $get_related = "select * from products where category_id=$category_id";

            $run_related = mysqli_query($con, $get_related);
            while ($row = mysqli_fetch_assoc($run_related)) {
            ...
            <div class='card' style='width: 90%'>
                ...
                <div class='card-body'>
                    ...
                    <p class='card-text'><strong>Stock: </strong>$product_stock</p>
                    <h5>₱ $product_price</h5>
                    <a href='index.php?add_to_cart=$row_product_id' style='background-color:#42C84F; border-color: #42C84F;' class='btn btn-green'>Add to Cart</a>
                    <a href='product_details.php?product_id=$row_product_id' class='btn btn-secondary'>View More</a>
                </div>
            </div>
        </div>";
                }
